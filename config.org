...
** Theme
   Theme loading and org-bullets don't get along perfectly, and
   leading *'s usually get unhidden. This is solved simply by calling
   font-lock-fontify-buffer, so let's advise load-theme to do that for
   us.

   #+begin_src emacs-lisp
     (defun load-theme--fontify (theme &rest args)
       (font-lock-flush))
     (advice-add 'load-theme :after #'load-theme--fontify)
   #+end_src
...
** Custom
   ...
   #+RESULTS:
   : #s(hash-table size 65 test eql rehash-size 1.5 rehash-threshold 0.8125 data (:use-package (24390 52651 106679 0) :init (24390 52651 106664 0) :init-secs (0 0 86 0) :use-package-secs (0 5 794481 0)))
...
*** [[https://github.com/Chobbes/org-chef][Chobbes/org-chef: A package for making a cookbook and managing recipes with org-mode.]]
    #+begin_src emacs-lisp
      (use-package org-chef
        :config
        (add-to-list 'org-capture-templates '("c" "Cookbook" entry (file "~/Sync/Notes/Cookbook.org") "%(org-chef-get-recipe-from-url)"))
        (add-to-list 'org-capture-templates '("m" "Manual Cookbook" entry (file "~/Sync/Notes/Cookbook.org")
                                              "* %^{Recipe title: }\n  :PROPERTIES:\n  :source-url:\n  :servings:\n  :prep-time:\n  :cook-time:\n  :ready-in:\n  :END:\n** Ingredients\n   %?\n** Directions\n\n")))
    #+end_src
