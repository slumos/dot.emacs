...
#+KEYWORDS:  emacs org
#+LANGUAGE:  en
#+STARTUP: entitiespretty content
...
** Dash
   #+begin_src emacs-lisp
     (use-package counsel-dash
       :hook
       (elpy-mode . (lambda () (setq-local counsel-dash-docsets "Python_3"))))
       #+end_src
...
** Python
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package ein
       ...
       :custom
       ((ein:completion-backend . 'ein:use-ac-company-backend))
       :config
...
** Getting customized vars to copy to use-package blocks
   #+begin_src emacs-lisp :results value table drawer replace
     (let ((found nil))
           (mapatoms (lambda (sym)
   ...
   #+RESULTS:
   :results:
   | magit-clone-always-transient   | (t)                                                                | nil                                   |
   ...
   | magit-clone-default-directory  | (~/Projects)                                                       | nil                                   |
   | doom-modeline-icon             | (t)                                                                | Use all-the-icons                     |
