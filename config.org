# -*- mode: org; -*-
# Time-stamp: <2026-03-04 Wed 12:08 slumos@Steven-Lumos-VXXQV3F32R>
  ...
  (use-package exec-path-from-shell
    ...
    (exec-path-from-shell-copy-envs
     '("AWS_CONFIG_FILE"
       "EMAIL"
       "LANG"
       "OPENAI_API_KEY_AIDER"
       "OPENAI_EMACS_GPT_COMMIT_TOKEN"
       "OPENAI_EMACS_GPTEL_TOKEN"
       "SLUMOS_CIRCLECI_CLI_TOKEN"
...
#+BEGIN_SRC emacs-lisp
  (require 'xdg)
#+END_SRC
*** [[https://mise.jdx.dev/ide-integration.html#emacs][Mise integration]]
#+begin_src emacs-lisp
  (use-package mise
...
*** PragmataPro
#+BEGIN_SRC emacs-lisp
  (set-frame-font "PragmataPro Mono Liga-14")
#+END_SRC
...
#+end_src
** COMMENT [[https://github.com/jdtsmith/indent-bars][jdtsmith/indent-bars: Fast, configurable indentation guide-bars for Emacs]]
#+begin_src emacs-lisp
...
#+end_src
** COMMENT [[https://github.com/Ailrun/magit-lfs][Ailrun/magit-lfs: Git LFS for magit!]]
#+begin_src emacs-lisp
...
SCHEDULED: <2025-02-10 Mon>
  ...
  #+RESULTS:
  : #s(hash-table data (user "git" host "github.com" org "slumos" repo "dot.emacs"))
...
** Worktree
#+begin_src emacs-lisp
  (defun slumos/abbreviate-directory-name (dir-name)
    "Abbreviate DIR-NAME by taking first letter of each hyphen-separated word.
  Example: quizlet-infrastructure -> qi"
    (mapconcat (lambda (word) (substring word 0 1))
               (split-string dir-name "-")
               ""))

  (defun slumos/create-worktree-for-workitem (workitem-id tag)
    "Create a worktree and branch for a Jira WORKITEM-ID with TAG.
  Branch: slumos-WORKITEM-ID-TAG
  Worktree: <parent-dir>/<repo-abbrev>_WORKITEM-ID-TAG"
    (interactive
     (list (read-string "Workitem ID: ")
           (read-string "Tag: ")))
    (let* ((repo-dir (magit-toplevel))
           (repo-name (file-name-nondirectory (directory-file-name repo-dir)))
           (parent-dir (file-name-directory (directory-file-name repo-dir)))
           (repo-abbrev (slumos/abbreviate-directory-name repo-name))
           (branch-name (format "slumos-%s-%s" workitem-id tag))
           (worktree-name (format "%s_%s-%s" repo-abbrev workitem-id tag))
           (worktree-path (expand-file-name worktree-name parent-dir))
           (start-point (magit-get-current-branch)))
      (magit-worktree-branch worktree-path branch-name start-point)))

  (with-eval-after-load 'magit-worktree
    (transient-append-suffix 'magit-worktree "b"
      '("w" "Workitem" slumos/create-worktree-for-workitem)))
#+end_src

** Initialization
** TRYING [[https://github.com/jonathanchu/magit-gh][jonathanchu/magit-gh: A lightweight GitHub CLI (gh) integration for Magit]]
#+begin_src emacs-lisp
  (use-package magit-gh :after magit)
#+end_src
...
** Config
   ...
   #+begin_src emacs-lisp
     (add-hook 'org-mode-hook #'magit-wip-mode)
   #+end_src

   
   I /think/ I want =visual-line-mode= always in org... still thinking about it.
...
*** COMMENT [[https://github.com/magit/orgit][magit/orgit: Support for Org links to Magit buffers]]
    #+begin_src emacs-lisp
      (use-package orgit)
    #+end_src
*** COMMENT [[https://github.com/akirak/ivy-omni-org][akirak/ivy-omni-org: An Ivy command which lets you visit Org buffers, files, bookmarks, and agenda commands]]
    #+begin_src emacs-lisp
      (leaf ivy-omni-org
        :after counsel
...
#+begin_src emacs-lisp
  ...
  (defun slumos/org-find-workitem-for-subtree ()
    (cl-loop for headline in (slumos/org-subtree-headlines)
             ...
             if (and title (string-match "\[A-Z\]\+-\[0-9\]\+" title)) return (match-string 0 title)))
...
#+begin_src emacs-lisp
  (use-package syncthing
    ...
    :config
    (if-let ((syncthing-token (cdar (exec-path-from-shell-copy-envs '("SYNCTHING_TOKEN")))))
        (setq syncthing-default-server-token syncthing-token)))
#+end_src
* Language Modes
** flymake
#+begin_src emacs-lisp
...
** TODO COMMENT [[https://github.com/christophstockhusen/bigquery-mode][christophstockhusen/bigquery-mode: GNU Emacs Major Mode for Editing and Executing BigQuery Scripts]]
#+begin_src emacs-lisp
  (use-package bigquery-mode
    :vc (:fetcher github :repo christophstockhusen/bigquery-mode))
#+end_src
...
#+begin_src emacs-lisp
  (use-package flycheck
    :hook (puppet-mode ruby-mode json-mode python-mode terraform-mode))
#+end_src
...
#+begin_src emacs-lisp
  (setq slumos-eglot-k8s-config '(:yaml :schemas
                                   (:Kubernetes ("pod.yaml"
                                                 "service.yaml"
                                                 "deployment.yaml"
                                                 "configmap.yaml")
...
#+end_src
** Markdown
*** TRYING [[https://github.com/hyperZphere/mark-graf][hyperZphere/mark-graf: Emacs Markdown mode with inline rendering]]
#+begin_src emacs-lisp
  (use-package mark-graf
    :commands mark-graf-mode
    :mode (("\\.md\\'" . mark-graf-mode))
    :hook (mark-graf-mode . (lambda () (display-line-numbers-mode -1))))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (use-package terraform-mode
    :config
    (setq lsp-disabled-clients '(semgrep-ls tfls))
                                          ;:hook
                                          ; see apheleia (terraform-mode . terraform-format-on-save-mode)
    )
   #+END_SRC

*** COMMENT [[https://github.com/kgrotel/terraform-ts-mode][kgrotel/terraform-ts-mode: Terraform major mode with treesit and eglot support]]
Not there yet based on my experience. [2023-12-06 Wed]
...
** YAML
   ...
   #+begin_src emacs-lisp :tangle no
     (use-package eglot-yaml
       :after (eglot)
       :vc (:fetcher github :repo buzztaiki/eglot-yaml)
       :init
...
#+begin_src emacs-lisp :tangle no
  (defcustom slumos/eglot-yaml-local-schema-base-dir "/opt/projects/json-schemas"
    "A local directory holding schemas in :group/:kind_:version.json layout"
    :group 'slumos
    :type 'string)

  (defun slumos/eglot-yaml-local-schema-resolver ()
    "Check local schema location first"
    (pcase-let* ((api-version (save-excursion (and (re-search-forward "^apiVersion:[ \t]*\\([^ \t\n]+\\).*$" nil t) (match-string 1))))
                 (`(,group ,version) (string-split api-version "/"))
                (kind (save-excursion (and (re-search-forward "^kind:[ \t]*\\([^ \t\n]+\\).*$" nil t) (match-string 1))))
                (local-schema-path (format "%s/%s/%s_%s.json" slumos/eglot-yaml-local-schema-base-dir (downcase group) (downcase kind) version)))
      (if (file-exists-p local-schema-path) local-schema-path nil)))

  ;; (with-current-buffer "debeziumserver.yaml"
  ;;   ;(slumos/eglot-yaml-local-schema-resolver))
#+end_src

* LLM
** TRYING [[https://github.com/stevemolitor/claude-code.el][stevemolitor/claude-code.el: Claude Code Emacs integration]]
*** Dependencies
Just dumping dependencies here for now
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package claude-code
    ...
    :custom
    (claude-code-program-switches '("--settings" "{\"prefersReducedMotion\": true}"))
...
** COMMENT [[https://github.com/manzaltu/claude-code-ide.el][manzaltu/claude-code-ide.el: Claude Code IDE integration for Emacs]]
#+begin_src emacs-lisp
(use-package claude-code-ide
  :vc (:fetcher github :repo manzaltu/claude-code-ide.el :rev :newest)
  :bind ("C-c C-'" . claude-code-ide-menu)
  :config
  (claude-code-ide-emacs-tools-setup))
#+end_src

** COMMENT [[https://github.com/MatthewZMD/aidermacs][MatthewZMD/aidermacs: AI Pair Programming in Emacs with Aider]]
#+begin_src emacs-lisp
  (use-package aidermacs
    :bind (("C-c SPC" . aidermacs-transient-menu))
    :config
    (setenv "OPENAI_API_KEY" (getenv "OPENAI_API_KEY_AIDER"))
    :custom
    (aidermacs-backend 'vterm))
#+end_src

** COMMENT [[https://github.com/copilot-emacs/copilot.el][copilot-emacs/copilot.el: An unofficial Copilot plugin for Emacs.]]
#+begin_src emacs-lisp
  (use-package copilot
    :vc (:fetcher github
    ...
    ("M-C-<backspace>" . copilot-cancel-completion))
#+end_src
** COMMENT [[https://github.com/chep/copilot-chat.el][chep/copilot-chat.el: Chat with Github copilot in Emacs !]]
#+begin_src emacs-lisp
  (use-package copilot-chat
    :bind ("C-c p" . copilot-chat-transient))
#+end_src
*** Add go files under current directory
#+begin_src emacs-lisp
  (defun slumos/copilot-chat-add-golang-files-current-dir (dir)
    (interactive "D")
    (let* ((max-files 40)
           (files (directory-files-recursively dir (rx ".go" eos))))
      (message "got files in %s: %S" dir files)
      (if (> (length files) max-files)
          (message "Too many files (%d > %d). Skipping." (length files) max-files)
        (dolist (file files)
          (copilot-chat-add-file file))
        (message "Added %d files." (length files)))))
#+end_src

** COMMENT [[https://github.com/karthink/gptel][karthink/gptel: A simple LLM client for Emacs]]
#+begin_src emacs-lisp
  (use-package gptel
...
;;    llava:latest
;;    mistral:latest)
** TODO COMMENT [[https://github.com/daedsidog/evedel][daedsidog/evedel: Instructed LLM programmer/assistant for Emacs]]
#+begin_src emacs-lisp
(use-package evedel
  :defer t
  :config
  (customize-set-variable 'evedel-empty-tag-query-matches-all nil)
  ;; These conflict with flymake so have to modify before use
...
#+end_src
** TODO COMMENT [[https://github.com/lanceberge/elysium][lanceberge/elysium: Automatically apply AI-generated code changes in Emacs]]
** TODO COMMENT [[https://github.com/karthink/gptel/wiki/Defining-custom-gptel-commands#example-2-rewrite-a-region][Defining custom gptel commands · karthink/gptel Wiki]]
* Helpers
** TODO [[https://github.com/emacselements/whisper][emacselements/whisper: An Emacs package that provides sophisticated speech-to-text functionality using Whisper]]
** TRYING edit-server (with [[https://github.com/stsquad/emacs_chrome][stsquad/emacs_chrome]])
#+begin_src emacs-lisp
...
** TRYING [[https://github.com/twlz0ne/separedit.el][twlz0ne/separedit.el: Edit comment or string/docstring or code block inside them in separate buffer with your favorite mode]]
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package highlight-indent-guides
    ...
    :hook (yaml-mode yaml-pro-mode yaml-pro-ts-mode))
#+end_src

** [[https://github.com/Fanael/edit-indirect/][Fanael/edit-indirect]]
#+begin_src emacs-lisp
  (use-package edit-indirect)
#+end_src

** [[https://github.com/federicotdn/verb][federicotdn/verb: Organize and send HTTP requests from Emacs]]
#+begin_src emacs-lisp
  (use-package verb
    :after org
...
| key | path                                     | name                     | fun          |
|-----+------------------------------------------+--------------------------+--------------|
| a   | ~/.config/alacritty/alacritty.toml       | alacritty config         | chezmoi-find |
| c   | ~/.emacs.d/config.org                    | main config (config.org) | find-file    |
| n   | ~/Sync/ST/Notes/Snippets.org             | snippets                 | find-file    |
| N   | ~/Sync/OneDrive/Notes/NOTES.org          | NOTES.org                | find-file    |
| p   | ~/.emacs.d/personal.org                  | personal config          | find-file    |
| q   | ~/Notes/Quizlet.org                      | Quizlet.org              | find-file    |
| r   | ~/Sync/ST/Notes/Cookbook.org             | cookbook                 | find-file    |
| s   | ~/Sync/ST/Notes/SLUMOS.org               | SLUMOS.org               | find-file    |
| t   | ~/Sync/ST/Notes/TODO.org                 | TODO.org                 | find-file    |
...
| z   | ~/.zshrc                                 | zshrc                    | chezmoi-find |
     ...
     #+begin_src emacs-lisp :noweb yes
       (defhydra hydra-commonly-visited-files-shortcuts (:color blue :columns 1)
         "visit file"
...
*** avy
    ...
    #+END_SRC
*** [[https://github.com/kickingvegas/casual-avy][kickingvegas/casual-avy: Casual Avy - An opinionated Transient menu for Avy.]]
#+begin_src emacs-lisp
...
*** [[https://github.com/noctuid/link-hint.el][noctuid/link-hint.el: Pentadactyl-like Link Hinting in Emacs with Avy]]
    ...
    #+end_src
** [[https://github.com/junyi-hou/tree-sitter-fold][junyi-hou/tree-sitter-fold: Code-folding using tree-sitter]]
#+begin_src emacs-lisp
    (use-package tree-sitter-fold
      :vc (:fetcher github :repo junyi-hou/tree-sitter-fold :rev switch-to-treesit)
      :bind ("C-<return>" . tree-sitter-fold-toggle))
#+end_src
...
** [[https://github.com/r0bobo/kubedoc.el][r0bobo/kubedoc.el: kubedoc.el - Kubernetes API Documentation]]
#+begin_src emacs-lisp
  (use-package kubedoc)
#+end_src
...
#+begin_src emacs-lisp
    (use-package apheleia
      ...
      (terraform-ts-mode . apheleia-mode))
#+end_src

** COMMENT [[https://github.com/emacs-sideline/sideline][emacs-sideline/sideline: Show information on the side]]
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
    (use-package sideline-load-cost :after sideline
      :vc (:fetcher github :repo emacs-sideline/sideline-load-cost))
#+end_src

** [[https://github.com/tuh8888/chezmoi.el][tuh8888/chezmoi.el: An emacs package for interacting with chezmoi.]]
#+begin_src emacs-lisp
  (use-package chezmoi
    :vc (:fetcher github :repo tuh8888/chezmoi.el)
    :load-path "~/.emacs.d/elpa/chezmoi/extensions"
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun rename-file-and-buffer ()
    "Reminder to try Emacs built-in"
    (interactive)
    (error "Try `rename-visited-file`"))
#+END_SRC
...
** TODO [[https://github.com/GarySGlover/dotfiles/blob/6469ea9b5aeb64ff5ebee53713eb3ba22bdda818/modules/users/global/editors/emacs/emacs-config.org#transient][dotfiles/modules/users/global/editors/emacs/emacs-config.org at 6469ea9b5aeb64ff5ebee53713eb3ba22bdda818 · GarySGlover/dotfiles]]
...
#+begin_src emacs-lisp :results value replace
  (let* ((branch-name "tmp-branch")
         (base-dir "~/.local/worktrees")
         (worktree-dir (expand-file-name branch-name base-dir)))
    worktree-dir)
    
#+end_src

#+RESULTS:
: /Users/slumos/.local/worktrees/tmp-branch
