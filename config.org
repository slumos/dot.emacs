# -*- mode: org; org-confirm-babel-evaluate: nil; fill-column: 120 -*-
...
*** =use-package=
    ...
    #+end_src
*** straight
    I am so over package authors pushing their favorite non-package.el
    packaging system onto me. Why do you want your packages to have a
    dependency like that?

    =straight.el= in particular requires all this crap. Why can't at least the packaging system du jour be installable by =package.el=?
...
*** [[https://github.com/cryon/almost-mono-themes][almost-mono]]
    ...
    #+end_src
*** COMMENT [[https://github.com/jannis/emacs-constant-theme][constant]]
    #+begin_src emacs-lisp
...
** Misc
   ...
   #+begin_src emacs-lisp
   (custom-set-variables '(frame-resize-pixelwise t))
   #+end_src

* Helpers
** TRYING [[https://github.com/ibizaman/emacs-conflict][ibizaman/emacs-conflict: Resolve conflicts happening when using file synchronization tools]]
   #+begin_src emacs-lisp
...
** [[https://github.com/Wilfred/helpful][Wilfred/helpful: A better Emacs *help* buffer]]
   #+begin_src emacs-lisp
     (use-package helpful
       :after counsel
       :custom
       (counsel-describe-function-function #'helpful-callable "Enhanced describe-function")
       (counsel-describe-function-variable #'helpful-variable "Enhanced describe-variable"))
   #+end_src
...
** [[https://github.com/politza/pdf-tools][PDF-Tools: Emacs support library for PDF files.]]
   #+begin_src emacs-lisp
    ...
    #+end_src
** COMMENT [[https://github.com/fuxialexander/org-pdftools][fuxialexander/org-pdftools: A custom org link type for pdf-tools]]
   #+begin_src emacs-lisp
...
** Initialization
   ...
   Enable using =add-to-list= with =org-capture-templates=
   #+begin_src emacs-lisp
   (setq org-capture-templates nil)
   #+end_src


   Took a while to warm up to =visual-line-mode= but I've come to like it with =org-mode=.
   #+begin_src emacs-lisp
   (use-package visual-line-mode :hook org-mode :ensure nil)
   #+end_src
...
** Config
   ...
   Decided to switch to yas-mode, and it's come in handy in hand for
   many other things. One favorite use is to make repeatable
   checklists in org. Unfortunately, those are hidden away in my
   ...
   #+RESULTS:
   | (lambda nil (yas-minor-mode 1) (yas-reload-all)) | org-bullets-mode | (lambda nil (magit-wip-after-save-mode t) (magit-wip-after-apply-mode t) (magit-wip-before-change-mode t)) | #[0 \301\211\207 [imenu-create-index-function org-imenu-get-tree] 2] | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-show-all append local] 5] | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-babel-show-result-all append local] 5] | org-babel-result-hide-spec | org-babel-hide-all-hashes | org-eldoc-load |
...
** Custom
   ...
   This directory is synchronized between all my devices using Syncthing, which I have not really got the hang of yet
   and end up with too many conflict files constantly. Need to figure this out.

   #+begin_src emacs-lisp
     (org-directory (expand-file-name "~/Sync/Notes"))
   #+end_src


   Regular ellipsis drives me insane because I tend to end regular lines that way all the time ... (see?)
      ...
      (use-package org-chef
        ...
        (setq org-capture-templates
              (append org-capture-templates
                      '(("r" "Recipe from:")
                        ("rc" "Chrome" entry (file "Cookbook.org") "%(slumos-recipe-from-chrome)" :jump-to-captured t)
                        ("ru" "URL" entry (file "~/Sync/Notes/Cookbook.org") "%(org-chef-get-recipe-from-url)" :jump-to-captured t)
                        ("rh" "Hand" entry (file "~/Sync/Notes/Cookbook.org")
                         "* %^{Recipe title: }\n  :PROPERTIES:\n  :source-url:\n  :servings:\n  :prep-time:\n  :cook-time:\n  :ready-in:\n  :END:\n** Ingredients\n   %?\n** Directions\n\n")))))
    #+end_src
...
** Getting customized vars to copy to use-package blocks
   ...
   | org-ql-search-directories-files-regexp | (\\.org\\(_done\\)?\\')                                             | nil                                   |
