# -*- mode: org; -*-
# Time-stamp: <2022-03-14 Mon 15:19 slumos@stevenlum-ltm>
...
*** Config
   ...
   Finally experimenting with mixed fonts!
   #+begin_src emacs-lisp
...
** [[https://github.com/kubernetes-el/kubernetes-el][kubernetes-el/kubernetes-el: Manage Kubernetes clusters with Emacs.]]
    #+begin_src emacs-lisp
      (use-package kubernetes
        :commands (kubernetes-overview)
        :init
        ;; workaround https://github.com/kubernetes-el/kubernetes-el/issues/265
...
#+begin_src emacs-lisp
  (defhydra slumos-hydra-copy-something-as-kill (:color blue)
    "copy as kill"
      ...
      ("w" slumos-copy-workitem-id-for-subtree "workitem for subtree (org-mode)"))
     ...
     (defun slumos-org-copy-to-clipboard ()
       ...
       (let* ((backend (or (org-export-get-backend 'slimhtml)
         ...
         (with-current-buffer html-buf
           ...
           (bury-buffer))))
   #+end_src
** DONE crazy find and copy workitem
#+begin_src emacs-lisp
  (defun slumos-copy-workitem-id-for-subtree ()
    "Find a workitem id somewhere in the current tree and copy it to the clipboard"
    (interactive)
    (let ((wid (slumos-crazy-find-workitem-id-for-subtree)))
      (if wid
          (progn (kill-new wid)
                 (message "%s" wid))
        (user-error "Didn't find a WorkItem"))))

  (defun slumos-crazy-find-workitem-id-for-subtree ()
    (let ((headlines (slumos-get-current-subtree-headlines))
          (workitem))
      (cl-loop for headline in headlines do
               with parent = (org-element-property :parent headline)
               with title = (or (org-element-property
               (let* ((parent (org-element-property :parent headline))
                      (title (or (org-element-property :raw-value headline)
                                 (org-element-property :raw-value parent))))
                 if (string-match "W-\[0-9\]\+" title)
                 return (match-string 0 title)))))

  (defun slumos-get-current-subtree-headlines ()
    "Return a list of headings in the current subtree"
    (save-excursion
      (let ((headings))
        (unless (org-at-heading-p) (outline-previous-visible-heading 1))
        (if (org-at-heading-p) (push (org-element-context) headings))
